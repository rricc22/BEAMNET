...
run_step 2 "Mesh reference beam (CalculiX format)" \
    ""$FREECADCMD" '$SCRIPT_DIR/scripts/02_MESH_REFERENCE_BEAM.py'"

run_step 3 "Generate CalculiX .inp files (1575 cases)" \
    "python3 '$SCRIPT_DIR/scripts/03_GENERATE_CCX_INPUTS_ELASTIC.py'"

run_step 4 "Run CalculiX simulations + convert to VTU/VTK" \
    "python3 '$SCRIPT_DIR/scripts/04_RUN_AND_CONVERT.py'"

run_step 5 "Generate FEM displacement overview video" \
    "python3 '$SCRIPT_DIR/utils/make_video.py'"

run_step 6 "Train axial beam PINN" \
