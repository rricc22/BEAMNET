...
Generate the reference beam geometry: 1 × 1 × 10 cube ratio.
  ...
  Nfix          : face at X = 0  (clamped / encastre end)
  Nload         : face at X = L  (free / loaded end)
...
def main():
    ...
    print("=" * 55)
    print("Reference Beam  (1 × 1 × 10 cube ratio)")
    print(f"  Cross-section : {a:.0f} × {a:.0f} mm  (Y-Z plane)")
    print(f"  Length        : {L:.0f} mm  (along X-axis)")
    print(f"  Nfix          : X = 0  (clamped / encastre)")
    print(f"  Nload         : X = {L:.0f}  (free / loaded end)")
    print("=" * 55)
    ...
    meta = {
        "name": "reference_beam",
        "description": f"Square-section prismatic beam {L:.0f}x{a:.0f}x{a:.0f} mm, axis X, cantilever",
        "a_mm": a,
        "L_mm": L,
        "ratio": [1, 1, 10],
        "axis": "X",
    ...
    obj = (
        FreeCAD.ActiveDocument.addObject("Part::Feature", "ReferenceBeam")
        if FreeCAD.ActiveDocument
        else FreeCAD.newDocument().addObject("Part::Feature", "ReferenceBeam")
    )
    obj.Shape = shape
